...
// The original file is fully working.
// Let's just find `return (` and replace it. Wait, the original `return (` is exactly:
/*
  return (
    <div className={styles.mrWorkspaceWrap}>
      <div className={styles.mrWorkspaceHeader}>
*/

// I will ONLY replace the JSX returned by `MRCaseWorkspaceClient`.
// The start of the return is: `return (\n    <div className={styles.mrWorkspaceWrap}>`
// But there is another early return:
//   if (!c) {
    ...
    <div className={styles.modernContainer}>
      ...
      <div className={styles.modernTabs}>
        {tabs.map(tab => (
          <div
            key={tab.id}
            className={\`\${styles.modernTab} \${active === tab.id ? styles.modernTabActive : ''}\`}
          ...
          <div className={\`\${styles.modernCard} \${styles.animateFadeIn}\`}>
            ...
            <div className={styles.modernGrid3}>
              ...
              <div className={styles.modernFormGroup}>
                ...
                <span className={styles.modernValue}>₹{c.advances.reduce((acc, a) => acc + (Number(a.amount) || 0), 0).toLocaleString()}</span>
              </div>
            </div>
          </div>
        )}
          ...
          <div className={styles.mrStagePanel}>
            ...
            <div className={styles.mrA4Preview}>
              ...
              <div className={styles.mrA4Body}>
                ...
                <p><strong>4. Advance Taken:</strong> ₹{c.advances.reduce((acc, a) => acc + (Number(a.amount) || 0), 0).toLocaleString()}</p>
                <p><strong>5. Net Payable:</strong> ₹{(totalClaimAmount - c.advances.reduce((acc, a) => acc + (Number(a.amount) || 0), 0)).toLocaleString()}</p>
              </div>
            </div>
          </div>
        )}
      ...
      <div className={styles.mrStepNavigator}>
        <button className={styles.modernBtnSecondary} onClick={() => {
            const currentIndex = tabs.findIndex((t) => t.id === active);
            if (currentIndex > 0) setActive(tabs[currentIndex - 1].id);
        }} disabled={tabs.findIndex((t) => t.id === active) === 0}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#64748b', fontWeight: 500 }}>
          ...
          <span>Step {tabs.findIndex((t) => t.id === active) + 1} of {tabs.length}</span>
        </div>
        <button className={styles.modernBtnSecondary} onClick={() => {
            const currentIndex = tabs.findIndex((t) => t.id === active);
            if (currentIndex < tabs.length - 1) setActive(tabs[currentIndex + 1].id);
        }} disabled={tabs.findIndex((t) => t.id === active) === tabs.length - 1}>
          Next
      ...
      <div className={styles.mrStickyActionBar}>
        <div className={styles.mrStickyActionStatus}>
          ...
          <small style={{ display: 'block', color: '#94a3b8' }}>{tabs.find((t) => t.id === active)?.label}</small>
        </div>
