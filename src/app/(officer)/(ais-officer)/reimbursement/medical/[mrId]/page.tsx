...
export default function MedicalCaseWorkspace() {
  const params = useParams<{ mrId: string }>();
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900">{params.mrId}</h1>
      ...
      {advanceModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/30 p-4">
          <form onSubmit={onSubmitAdvance} className="w-full max-w-lg rounded-xl bg-white p-5 shadow-2xl">
            <div className="mb-4 flex items-center justify-between"><h3 className="text-lg font-semibold">Request Advance for {params.mrId}</h3><button type="button" onClick={() => setAdvanceModal(false)}><XMarkIcon className="h-5 w-5" /></button></div>
            <div className="space-y-3 text-sm">
      ...
      {previewModal && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/30 p-4">
          <div className="mx-auto mt-8 w-full max-w-4xl rounded-xl bg-white p-6 shadow-2xl">
            <div className="mb-4 flex items-center justify-between"><h3 className="text-xl font-semibold">Submit Final Claim: {params.mrId}</h3><button onClick={() => setPreviewModal(false)}><XMarkIcon className="h-5 w-5" /></button></div>
            <div className="grid gap-4 md:grid-cols-2 text-sm">
